...
def initialize_ema_one(data_center, ts_code):
    ...
    temp_ema = ta.ema(close_series, length=45)
    write_data_frame['ema_45'] = temp_ema
    temp_ema = ta.ema(close_series, length=50)
    write_data_frame['ema_50'] = temp_ema
    temp_ema = ta.ema(close_series, length=55)
    write_data_frame['ema_55'] = temp_ema
    temp_ema = ta.ema(close_series, length=60)
    write_data_frame['ema_60'] = temp_ema
...
def append_cal_ema():
    ...
    for item in stock_list:
        ...
        column_list.append("ema_45")
        column_list.append("ema_50")
        column_list.append("ema_55")
        column_list.append("ema_60")
        ...
        for i in range(len(base_infos)):
            ...
            temp_dict["ema_45"] = cal_ema_single_day(last_ema_val, curr_close, 45)
            temp_dict["ema_50"] = cal_ema_single_day(last_ema_val, curr_close, 50)
...
def append_cal_vol_ema():
    ...
    for item in stock_list:
        ...
        for i in range(len(base_infos)):
            ...
            curr_close = base_infos.at[i, 'vol']
            temp_dict['trade_date'] = base_infos.at[i, 'trade_date']
